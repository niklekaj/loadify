...
export default {
  ...
  setup() {
    ...
    const updateTourFormDriverField = (event: Event, tourFormId: number) => {
      ...
      tourForms.value[tourFormId - 1].assignedDriver.id =
        drivers.value.find((d) => d.name === target.value)?.id || null
    }
...
<template>
  <section>
    <form
      v-for="(tourForm, index) in tourForms"
      :key="index"
      @submit.prevent="saveTourForms(tourForm.id)"
      class="tour"
    >
      ...
      <div class="tour_field">
        ...
        <select @input="updateTourFormDriverField($event, tourForm.id)">
          <option value="" disabled :selected="!tourForm.assignedDriver.name">
            Select an option
          </option>
          <option
            v-for="(driver, index) in allowedDriversForTour(tourForm.locationFrom)"
            :key="index"
            :value="driver.name"
            id="driverName"
          >
      ...
      <div class="tour_buttons">
        ...
        <button class="delete" type="button" @click="deleteTourForm(tourForm.id)">
          Delete Tour
        </button>
      </div>
